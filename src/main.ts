...
let onlyCakes: boolean = false;
let onlyCupcakes: boolean = false;
let currentSort: number = 0;
...
class Products {
  async getProducts() {
    try {
      // This API call will request entries with the space ID from the client variable, using a space-specific access token
      let products;
...
class Display {
  sortProducts(products: []) {
    ...
    // 2 is price, low to high
      ...
      products = products.sort((a: {price: number[]}, b: {price: number[]}) => a.price[0] > b.price[0] ? 1 : -1);
    // 3 is price, high to low
    } else if (currentSort === 3) {
      products = products.sort((a: {price: number[]}, b: {price: number[]}) => a.price[0] < b.price[0] ? 1 : -1);
    // 4 is alphabetical a-z
    } else if (currentSort === 4) {
      products = products.sort((a: {title: string}, b: {title: string}) => a.title > b.title ? 1 : -1);
    // 5 is reverse alphabetical z-a
    } else if (currentSort === 5) {
      products = products.sort((a: {title: string}, b: {title: string}) => b.title > a.title ? 1 : -1);
    }
